...
RUN ["/bin/bash", "-c", "/etc/init.d/mysql start && mysqladmin -h localhost -u root -proot version && ./Knowage-${KNOWAGE_VERSION}-${KNOWAGE_RELEASE_DATE}.sh -q -console -varfile default_params.properties"]
