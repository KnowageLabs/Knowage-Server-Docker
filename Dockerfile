...
RUN ["/bin/bash", "-c", "mysql -u root -proot -e 'SELECT host, user FROM mysql.user' && mysql -uroot -proot -e 'USE mysql; UPDATE `user` SET `Host`='%' WHERE `User`='root' AND `Host`='localhost'; DELETE FROM `user` WHERE `Host` != "%" AND `User`='root'; FLUSH PRIVILEGES;'"]
