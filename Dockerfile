...
RUN ["/bin/bash", "-c", "/etc/init.d/mysql start && {mysql --verbose --help | grep bind-address} && mysql -u root -proot -h localhost -e 'SELECT host, user FROM mysql.user' && ./Knowage-${KNOWAGE_VERSION}-${KNOWAGE_RELEASE_DATE}.sh -q -console -Dinstall4j.debug=true -Dinstall4j.keepLog=true -Dinstall4j.logToStderr=true -Dinstall4j.detailStdout=true -varfile default_params.properties"]
